function [ amplitude,phase,freq ] = monopoleopc_BB(x,h0,e1r,e2r,freq)
...
%   is obtained from the fitting of the approach curve;
%   g is a complex number usually at about 0.7~0.8*Exp[i0.06]; It describes
...
v=240000;   %oscillation frequency of AFM tip unit Hz
...
nrr=sqrt(0.5*sqrt(e1r(i)^2+e2r(i)^2)+0.5*e1r(i)); 
%nrr is the real part of the far field refractive index of reference sample
kir=sqrt(0.5*sqrt(e1r(i)^2+e2r(i)^2)-0.5*e1r(i)); 
%kir is the imaginary part of the far field refractive index of reference sample
rfr=((nrr+1i*kir)^2*cos(inc)-sqrt((nrr+1i*kir)^2-sin(inc)^2))...
    /((nrr+1i*kir)^2*cos(inc)+sqrt((nrr+1i*kir)^2-sin(inc)^2)); 
%rfr is the reflection coefficient in far field of reference sample
...
SCR=(1+rfr)^2*alpharn;                               
%SCR is the near field scattering of reference sample
